...
public class ShooterController : MonoBehaviour
{
    ...
    private Transform spawnPoint; // 슈터의 스폰 포인트로 행성을 발사하기 위한 기준이 되는 좌표

    [SerializeField] private float shooterSpeed = 100f; // 슈터를 마우스로 이동했을 때, 속도 실수 값
    [SerializeField] private float shooterMinRange = 4.3f; // 2D 환경에서의 2차원 x, y 좌표 중 x 좌표에 한해 슈터의 이동 범위 제한을 위한 범위 최소 실수 값
    [SerializeField] private float shooterMaxRange = 4.3f; // 2D 환경에서의 2차원 x, y 좌표 중 x 좌표에 한해 슈터의 이동 범위 제한을 위한 범위 최대 실수 값
    ...
    private void OnMouseCursor() // 마우스 커서 입력을 통해 슈터를 이동시키는 메소드
    {
        ...
        if (transform.position.x > shooterMaxRange) // 슈터의 x좌표값이 shooterMaxRange보다 클 때
        {
            transform.position = new Vector3(shooterMaxRange, transform.position.y, 0); // 슈터의 위치를 shooterRange좌표와 슈터의 y좌표 z = 0 으로 고정한다
        }
        if (transform.position.x < shooterMinRange) // 슈터의 -x좌표값이 shooerMinRange보다 작을 때
        {
            transform.position = new Vector3(shooterMinRange, transform.position.y, 0); // 슈터의 위치를 shooterRange좌표와 슈터의 y좌표 z = 0 으로 고정한다 
        }
    }

    private void OnMouseInput() // 마우스 좌,우버튼 입력받는 메소드
    {
        if (OptionController.isStated)
        {
            return;
        }
    ...
    private void OnShoot(GameObject _planet) // 행성 발사 메소드
    {
        ...
        // 행성 게임 오브젝트에서 리지드바디2D 컴포넌트를 불러온 후 행성의 아래 방향을 기준으로 shootPower 만큼의 힘을 가한다
        ...
        spriteRender.sprite = PlanetDatabase.planetSprite[currentPlanetNumber];
    }
